import { createFileRoute } from "@tanstack/react-router";

/**
 * Single-use SSO ticket exchange used by the five child apps.
 *   POST /api/public/sso/exchange   { "ticket": "...", "app": "finance" }
 * Called by the CHILD APP SERVER only.
 * Auth: x-tpcamp-key must match TPCAMP_SSO_KEY_<APP> (preferred) or TPCAMP_SSO_KEY.
 * Fails closed; a ticket can only ever be redeemed once and only for its own app.
 */
export const Route = createFileRoute("/api/public/sso/exchange")({
  server: {
    handlers: {
      OPTIONS: async ({ request }) => {
        const { corsHeaders } = await import("@/lib/entitlement.server");
        return new Response(null, {
          status: 204,
          headers: corsHeaders(request.headers.get("origin")),
        });
      },
      POST: async ({ request }) => {
        const { corsHeaders, redeemTicket, verifyServerKey, DENIED } = await import(
          "@/lib/entitlement.server"
        );
        const headers = {
          ...corsHeaders(request.headers.get("origin")),
          "Content-Type": "application/json",
        };

        try {
          const body = (await request.json()) as { ticket?: unknown; app?: unknown };
          const ticket = typeof body.ticket === "string" ? body.ticket : "";
          const app = typeof body.app === "string" ? body.app : undefined;

          if (!verifyServerKey(request.headers.get("x-tpcamp-key"), app)) {
            return new Response(JSON.stringify(DENIED), { status: 401, headers });
          }

          // Entitlement is recomputed inside redeemTicket — never trusted from issue time.
          const result = await redeemTicket(ticket, app);
          if (!result) {
            return new Response(JSON.stringify(DENIED), { status: 401, headers });
          }
          return new Response(
            JSON.stringify({ ...result.entitlement, app_slug: result.app_slug }),
            { status: 200, headers },
          );
        } catch {
          console.error("SSO exchange failed");
          return new Response(JSON.stringify(DENIED), { status: 500, headers });
        }
      },
    },
  },
});
